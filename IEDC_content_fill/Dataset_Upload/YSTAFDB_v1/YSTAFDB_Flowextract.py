...
FilePath = WorkingFolder + 'YSTAFDB\\YSTAFDB_core_tables\\flows.csv'
...
for line in lines:
    ...
    if lc > 0: # ignore header
        ...
        if line != '':
            ...
            # Select dataset ID:
            try:
                DSMatchList.append(DSF_Ids[MatDSList.index(Mat)])
            except:
                DSMatchList.append(DSF_Ids[13])
            ...
            Comment =  'comment = {\'system_boundary_origin\':\'' + cols[3]
            Comment += '\',\'system_boundary_destination\':\'' + cols[4] +'\','
            Comment += '\'aggregate_subsystem_module_origin\':\'' + cols[5] +'\','
            Comment += '\'aggregate_subsystem_module_destination\':\'' + cols[6] +'\','
            Comment += '\'subsystem_origin\':\'' + cols[7] +'\','
            Comment += '\'subsystem_destination\':\'' + cols[8] +'\','
            Comment += '\'YSTADFDB_flow_label\':\'' + cols[9] +'\','            
  
            # Parse Flow labels:
            FlowLabels = cols[9].split('_')
            ...
            if len(DestinProcesshierarchy) == 4 and DestinProcesshierarchy[0] == '55': # FIX: For some Tungsten entries, no subsystem code is provided:
                DestinProcesshierarchy.insert(2,'5;1')
        ...
        # Value:
        try:
            NumVal  = np.float(cols[11])
        except:
            NumVal  = None
        try:
            ConcVal = np.float(cols[13])
            if ConcVal > 1:
        ...
        if cols[14] == 'mass fraction (w/w)':            
            Comment += '\'mass fraction of material in commodity\':\'' + cols[13] +'\','
        
        Comment += '\'reliability\':\'' + cols[18] +'\','
        Comment += '\'method\':\'' + cols[19] +'\','
        Comment += '\'YSTAFDB_publication_id\':\'' + cols[20] +'\','
        Comment += '\'notes\':\'' + cols[21] +'\','
        Comment += '\'YSTAFDB_flow_id\':\'' + cols[22] +'\','
        # comment finish
        Comment += '}'   
                
        # uncertainty    
        if cols[17] == 'plus minus':
            U1 = 4
            N2 = None
            if cols[16] == '%':
                U3 = np.float(cols[11]) * (100 - np.float(cols[15])) / 100
                U4 = np.float(cols[11]) * (100 + np.float(cols[15])) / 100
            if cols[16] == 'Gg':
                U3 = np.float(cols[11]) - np.float(cols[15])
                U4 = np.float(cols[11]) + np.float(cols[15])
        else:
            U1 = None
            U2 = None
            U3 = None
        ...
        # Add data into db:
        cur.execute(SQL,(DSMatchList[-1],Mat_ID,Comm_ID,Tim_ID,Reg_ID,Reg_ID,ProcO_ID,ProcD_ID,None,None,None,None,None,NumVal,\
