...
cur.execute("SELECT id,attribute1 FROM classifiation_items WHERE classification_id = %s ", C1id)
C1Tuples = cur.fetchall()
C1IDs    = [x[0] for x in C1Tuples]
C1Labels = [x[1] for x in C1Tuples]

cur.execute("SELECT id,attribute2 FROM classifiation_items WHERE classification_id = %s ", C2id)
